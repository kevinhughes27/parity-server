...
export default function Event(props: { event: PointEvent; isCallahan?: boolean }) {
  ...
  if (isCallahan) {
    return (
      <span>
        <CelebrationIcon fontSize="small" style={{ marginRight: 5 }} /> {event.firstActor} got a
        Callahan!
      </span>
    );
  }
  ...
  switch (event.type) {
    ...
    case 'DROP':
      return (
        <span>
          <CallMissedIcon fontSize="small" style={{ marginRight: 5 }} /> {event.firstActor} dropped
          it
        </span>
      );
    case 'PASS':
    ...
    case 'PULL':
      return <span>{event.firstActor} pulled</span>;
    default:
      return null;
  }
}
