...
export default function Points(props: { game: Game }) {
  ...
  const renderPoint = (point: Point, homeScore: number, awayScore: number, idx: number) => {
    ...
    const thrower = secondLastEvent.type === 'PASS' ? secondLastEvent.firstActor : null;

    const homeScored = includes(game.homeRoster, receiver);
    const teamName = homeScored ? game.homeTeam : game.awayTeam;

    const homeColor = homeColors[8];
    const awayColor = awayColors[8];
    const teamColor = homeScored ? homeColor : awayColor;

    const teamJsx = <strong style={{ color: teamColor }}>{teamName}</strong>;

    const breakPoint = includes(point.defensePlayers, receiver);

    const whatCopy = breakPoint ? 'Break Point' : 'Point';

    const iconJsx = breakPoint ? <BoltIcon fontSize="small" /> : null;

    const whoCopy = thrower ? `${thrower} to ${receiver}` : receiver;
        ...
        <ListItemButton onClick={() => handleClick(isExpanded, idx)}>
          <ListItemText>
            <div style={{ display: 'flex', flex: 1, justifyContent: 'space-between' }}>
              <span>
                {iconJsx} {whatCopy} {teamJsx} {whoCopy} {durationCopy}
              </span>
