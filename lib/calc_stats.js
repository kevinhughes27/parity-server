...
function processSwitchingDirectionEvents (events, stats) {
  for (let i in events) {
    switch (events[i][0]) {
      ...
      case 'Pull':
        addEvent(events[i][1], 'Pulls', 1, stats)
        break

      case 'D':
        addEvent(events[i][1], 'D-Blocks', 1, stats)
        break
    }
  }
}
...
function processPassingEvents (events, stats) {
  for (let i in events) {
    for (let j = 0; j < events[i].length; j += 2) {
      ...
      if (blank(events[i][j + 2])) {
        addEvent(events[i][j + 1], 'Pick-Ups', 1, stats)
      } else {
        addEvent(events[i][j + 1], 'Catches', 1, stats)
        addEvent(events[i][j + 3], 'Completions', 1, stats)
      }
    }
  }
}
...
function processOnFieldEvents (events, stats) {
  for (let i in events) {
    switch (events[i][0]) {
      ...
      case 'O+':
        addEvent(events[i][1], 'OPointsFor', 1, stats)
        break

      case 'O-':
        addEvent(events[i][1], 'OPointsAgainst', 1, stats)
        break

      case 'D+':
        addEvent(events[i][1], 'DPointsFor', 1, stats)
        break

      case 'D-':
        addEvent(events[i][1], 'DPointsAgainst', 1, stats)
        break
    }
  }
}
