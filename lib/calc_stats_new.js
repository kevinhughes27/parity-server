...
let calcStats = function (points: Array<Point>) {
  ...
  for (let point of points) {
    ...
    // temp fix - check for any duplicate points
    let matchingPoints = _.filter(points, (p) => _.isEqual(p, point))
    if (matchingPoints.length > 1) {
      throw new Error(`found duplicate point ${point.events[0].timestamp}`)
    }
...
function processEvents (events, stats) {
  ...
  events.forEach((ev, idx) => {
    ...
    if (isTurnOver(ev)) {
      ...
      if (nextPlay.type === 'DEFENSE') { nextPlay = events[idx + 2] }
      if (nextPlay.type !== 'POINT') { addStat(nextPlay.firstActor, 'Pick-Ups', 1, stats) }
    }
